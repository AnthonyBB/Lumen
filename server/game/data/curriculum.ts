/**
 * K-12 curriculum taxonomy — the two-level subject → subcategory system.
 *
 * Each of the four learning subjects is broken into subcategories that span
 * kindergarten (grade 0) through 12th grade.  Question banks in
 * server/game/data/questions/ tag every question with one of these ids.
 *
 * KEEP IN SYNC with the client copy: src/game/data/curriculum.ts
 * (the client duplicates this list for its subcategory picker UI).
 */
...
/** Fast lookup: subcategory id → Subcategory.  Used to validate client-sent ids. */
export const SUBCATEGORY_MAP: Record<string, Subcategory> = Object.fromEntries(
  CURRICULUM.map((s) => [s.id, s]),
);

/** Subcategories grouped by subject. */
